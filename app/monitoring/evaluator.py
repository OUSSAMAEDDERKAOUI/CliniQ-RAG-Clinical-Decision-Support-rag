...
def load_eval_dataset() -> dict:
    if not os.path.exists(EVAL_DATASET_PATH):
        logger.warning(f" Dataset introuvable : {EVAL_DATASET_PATH}")
        return {}
...
def evaluate_rag(question: str, answer: str, context: list, llm_model, run_id: str = None):
    ...
    if expected_answer:
        metrics += [
            ContextualPrecisionMetric(model=llm_model),  
            ContextualRecallMetric(model=llm_model),     
        ]
        print(f"  Métriques contextuelles activées")
    else:
        print(f" Pas d'expected_answer → métriques contextuelles ignorées")
    ...
    if run_id:
        ...
        logger.warning(" Aucun run_id fourni — métriques non loguées")
